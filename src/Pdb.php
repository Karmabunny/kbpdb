...
use karmabunny\pdb\Drivers\PdbMysqli;
...
use karmabunny\pdb\Models\PdbStatement;
use mysqli;
...
abstract class Pdb implements Loggable
{
    ...
    public function __construct($config)
    {
        ...
        // The config provides an override PDO or mysqli connection.
        if (
            $this->config->_override instanceof PDO
            or $this->config->_override instanceof mysqli
        ) {
            $this->connection = $this->config->_override;
        }
     ...
     * @param string $query The query to execute. Prefix a table name with a tilde (~) to automatically include the
     *        table prefix, e.g. ~pages will be converted to fwc_pages
     * @return PDOStatement|PdbStatement The prepared statement, for execution with {@see Pdb::execute}
     * @throws QueryException If the query execution or formatting failed
     * @throws ConnectionException If the connection fails
     */
     ...
     * return types other than 'pdo', 'null', 'count' or 'val' because the other types all return the values as arrays
     *
     * @param PDOStatement|PdbStatement $st The query to execute. Prepare using {@see Pdb::prepare}
     * @param array $params Parameters to bind to the query
     * @param string|array|PdbReturn $config a return type or config {@see PdbReturn}
     * @return array|string|int|null|PDOStatement|PdbStatement
     * @throws InvalidArgumentException If the return type isn't valid
     * @throws QueryException If the query execution or formatting failed
     * @throws ConnectionException If the connection fails
     */
    public function execute($st, array $params, $config)
    {
        ...
        try {
            ...
            if (stripos($st->queryString, 'INSERT') === 0) {
                if ($db instanceof mysqli) {
                    $this->last_insert_id = (int) $db->insert_id;
                }
                else {
                    $this->last_insert_id = (int) $db->lastInsertId();
                }
            }

            $this->logQuery($st->queryString, $params, $res);

        } catch (PDOException|mysqli_sql_exception $ex) {
            $ex = QueryException::create($ex, $db)
                ->setParams($params)
     ...
     *
     * @param PDOStatement|PdbStatement $st Statement to bind parameters to
     * @param array $params Parameters to bind
     */
    protected static function bindParams($st, array $params)
    {
     ...
     */
    public static function formatRs($rs, $type)
    {
